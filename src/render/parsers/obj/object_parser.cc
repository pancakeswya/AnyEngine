...
void ProcessPolygon(Data& data, const std::vector<Indices>& raw_indices) {
  ...
  if (const size_t indices_len = raw_indices.size(); indices_len == 4) {
    ...
    std::ranges::move(raw_indices, std::back_inserter(data.indices));
  }
}
...
void ParseMtlBuffer(const std::vector<char>& buffer, Data& data) {
  NewMtl new_mtl;
  bool found_d = false;

  const unsigned int read = buffer.size();
...
const char* ParseUsemtl(const char* ptr, Data& data) {
  const std::string use_mtl_name = GetName(&ptr);
...
Data ParseFromFile(const std::string& path) {
  ...
  for (;;) {
    size_t read = SDL_ReadIO(file, start, kBufferSize);
...
render::GeometryTransferer& Parser::Parse(const std::string& path, std::vector<std::string>& texture_paths) {
  Data data = ParseFromFile(path);

  texture_paths.reserve(data.mtl.size());
  for(const NewMtl& mtl : data.mtl) {
    texture_paths.push_back(mtl.map_kd);
  }
  transferer_ = GeometryTransferer(std::move(data));
  return transferer_;
}
